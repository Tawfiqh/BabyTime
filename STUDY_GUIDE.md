...
**`static/audioVisualizer.js`**: Defines an `AudioVisualizer` class that owns the full audio meter lifecycle. `start()` creates `AudioContext`, creates/configures `AnalyserNode`, connects either an HTML media element (`createMediaElementSource`) or a `MediaStream` (`createMediaStreamSource`), ensures the bars exist, and starts the requestAnimationFrame loop. `stop()` cancels the loop, closes audio resources, and clears the bars. The class exposes an `onLevelUpdate(rms, peak)` callback: each animation frame, after updating the frequency bars, it computes the current RMS and Peak values from the frequency data and fires the callback. `viewer.js` uses this to throttle updates to ~1s intervals, push them into an `AudioLevelChart`, and update the RMS/Peak bar display.

**`static/audioChart.js`**: A shared `AudioLevelChart` class used by both the Slow Viewer and Normal Viewer. Wraps a Chart.js line chart with two datasets (RMS in green, Peak in red). `push(rms, peak, date)` adds a data point and rolls off the oldest when the history exceeds `maxPoints`. The chart uses `animation: false` and `update('none')` for zero-delay rendering. Both viewers instantiate this class with a different `maxPoints`: 120 for the Slow Viewer (10 min at 5s intervals), 600 for the Normal Viewer (10 min at 1s intervals).
